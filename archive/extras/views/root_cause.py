...
def display_root_cause(spike_ts: str = None, window_min: int = 5):
    ...
    if not spike_ts:
        console.print(Panel(
            "  Provide a spike timestamp to begin analysis.\n"
            "  Use [bold yellow]--spike-ts ISO-TIMESTAMP[/bold yellow] in the CLI.",
            title="[bold yellow]RCA Required[/bold yellow]", title_align="left",
            border_style="yellow", expand=False,
        ))
        return

    try:
        dt = datetime.datetime.fromisoformat(spike_ts.replace("Z", ""))
        start = (dt - datetime.timedelta(minutes=window_min)).strftime("%Y-%m-%dT%H:%M:%SZ")
        end = (dt + datetime.timedelta(minutes=window_min)).strftime("%Y-%m-%dT%H:%M:%SZ")
    except Exception as e:
        console.print(f"[red]Error parsing timestamp:[/red] {e}")
        return

    hits = logs_for_spike(start, end)
    if not hits:
        console.print(Panel(
            f"  No logs found in the {window_min}m window around {spike_ts}.\n"
            "  Verify connectivity and indexing state.",
            title="[bold red]Analysis Results[/bold red]", title_align="left",
    ...
    for a in annotated:
        ...
        table.add_row(a["ts"], f"[{col}]{a['level'].upper()}[/{col}]", a["host"], msg)

    console.print(table)
    indicators = len([a for a in annotated if a["reason"]])
    console.print(f"\n[dim]Analyzed 100 hits around {spike_ts}. Found {indicators} potential indicators.[/dim]")
